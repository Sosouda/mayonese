...
namespace laba2 {
    ...
    template <typename T>
        void  Tree<T>  :: add(const  Tree & tree) {
        std::lock_guard<std::recursive_mutex> lock(_mtx);
    ...
    template <typename T>
        ...
        Tree<T>  subtree;
        subtree.root =cloneSubtree(subtreeRoot);
        return subtree;
    }
    ...
    template <typename T>
        bool  Tree<T>  :: operator == (const   Tree<T> & tree1) const  noexcept {
    ...
    template <typename T>
        void  Tree<T>  :: deleteNode(const  T & value) noexcept {
        std::lock_guard<std::recursive_mutex> lock1(_mtx);
    ...
    std :: ostream & operator<<(std :: ostream & os, const  Tree<TT>& tree) {
        std :: vector<TT> lpk  = tree.LPK();
        std::vector<TT> lkp = tree.LKP();
        std::vector<TT> klp = tree.KLP();
        std::cout << std::endl << " lpk:" << std::endl;
        for (auto i = lpk.begin(); i != lpk.end(); i++) {
            os <<" " << *i;
        }
        std::cout << std::endl << " lkp:" << std::endl;
        for (auto i = lkp.begin(); i != lkp.end(); i++) {
            os << " " << *i;
        }
        std::cout << std::endl << " klp:" << std::endl;
        for (auto i = klp.begin(); i != klp.end(); i++) {
            os  << " " << *i;
        }
        return os;
    }
